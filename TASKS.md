...
> **前端 CI**：`frontend-build-and-test` job 已上线（typecheck/lint/test/build，`--ignore-scripts` 兜 supply-chain 拦截）。**设为分支保护必需检查的动作待 Owner 批准**（自动模式拦截了未经请求的治理变更）。
> **(b) 段前置已备**：`docs/UI_DATA_REFERENCE.md`（PR #30）给设计者的数据契约。
> **后端跟进项（未做）**：auth `sanitizeUser` 字段窄于 `/users/me`（缺 phone/avatarUrl）。
> **后端跟进项（Review #27 挖出，登记不阻塞）**：auth 的 `sanitizeUser`（login/register 响应）比 `/users/me` 窄，缺 `phone`/`avatarUrl`，与共享 `User` DTO 不一致；建议补齐 auth 响应字段（小型后端任务，TASK-011 之后按需开）。
