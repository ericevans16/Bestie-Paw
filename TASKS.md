...
| TASK-011 | 逻辑层 TS 化（services.jsx→src/services/*.ts + 单测） | (a) | 待派发 | 📋 TODO（009、010 已就绪，可派发） |
| (b) | 视图逐页 `.jsx→.tsx` + 组件测试 + 设计 token | (b) | — | ⏸ 待 UI 方向定后再拆卡 |

> **前端 CI 已接入（2026-06-05）**：`.github/workflows/ci.yml` 增 `frontend-build-and-test` job（pnpm workspace 安装 → typecheck/lint/test/build）。当前**未列入分支保护必需检查**；待 TASK-011 收口时再 `gh api` 设为阻塞门禁。
