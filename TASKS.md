...
| 任务 | 标题 | 段 | 分配给 | 状态 |
|---|---|---|---|---|
| TASK-009 | 前端脚手架（Vite+TS+Vitest+ESLint+pnpm workspace） | (a) | Antigravity | ✅ DONE（PR #28 已合并 2026-06-05；virtual-module stitching interop，整改掉误提交的 node_modules/dist + 加 .gitignore 后合并） |
| TASK-010 | `packages/shared` 契约类型（枚举/信封/错误码/DTO） | (a) | Antigravity | ✅ DONE（PR #27 已合并 2026-06-05；架构师修正 ErrorCode 对齐真实后端后合并） |
| TASK-011 | 逻辑层 TS 化（services.jsx→src/services/*.ts + 单测） | (a) | Antigravity | ✅ DONE（PR #31 已合并 2026-06-06；11 单测；架构师修正 shared 入口指向 src 后绿） |
| (b) | 视图逐页 `.jsx→.tsx` + 组件测试 + 设计 token | (b) | — | ⏸ 待 UI 方向定后再拆卡（设计输入见 `docs/UI_DATA_REFERENCE.md`） |

> **🎉 Phase 2 (a) UI 无关地基已闭环（2026-06-06）**：脚手架 + 共享类型 + 逻辑层 TS 化 + 前端 CI 全部落地。逻辑层 `app/services.jsx` 已拆为 `bestie-paw-frontend/src/services/*.ts`，用 `@bestiepaw/shared` 标注；枚举/信封手工适配器**暂留**（视图仍用小写枚举，删除需动 JSX → 留给 (b)）。
> **前端 CI**：`frontend-build-and-test` job 已上线（typecheck/lint/test/build，`--ignore-scripts` 兜 supply-chain 拦截）。**已设为 `main` 分支保护必需检查**（Owner 已批准 2026-06-06；现 main 必需 `build-and-test` + `frontend-build-and-test`）。
> **运行时验证**：浏览器冒烟通过（PR #33）；修复 stitch 漏注入 `ReactDOM.createPortal` 致 modal 白屏的全应用崩溃（源自 TASK-009 脚手架）。
> **(b) 段前置已备**：`docs/UI_DATA_REFERENCE.md`（PR #30）给设计者的数据契约。
> **后端跟进项**：auth `sanitizeUser` 缺 `phone`/`avatarUrl` → ✅ 已修复（PR #34，对齐 `/users/me` + shared `User` DTO）。

## 🔄 产品变更 — 社交圈 → 养宠好文
| 任务 | 标题 | 分配给 | 状态 |
|---|---|---|---|
| TASK-012 | 社区(community) 替换为 养宠好文(articles) + ADMIN 鉴权 | Claude Code | ✅ DONE（PR #35 已合并 2026-06-08；后端+契约+迁移） |

> **范围**：后端 + `packages/shared` + Prisma 迁移 + 后端文档。删 `Post`/`PostLike`/`Comment` 与 `community` 模块；加 `Article`/`ArticleLike`/`ArticleFavorite` 与 `articles` 模块（任意用户：阅读/点赞/收藏；维护者 ADMIN：发布/改/删，经 `requireAdmin` 从 DB 读 `User.role`）。**无评论**。新增 `enum Role{USER ADMIN}` + `User.role`；`sanitizeUser` 输出加 `role`。
> **⚠️ Owner 运维待办**：破坏性迁移 `20260608000000_replace_community_with_articles` 对**现网/dev 库**需 Owner 手动 `prisma migrate deploy`——会 **DROP 三张社区表、社区历史数据丢失**（全新库无碍）。seed 含 ADMIN 账号 `admin@bestiepaw.com`（默认口令见 PR #35，上线前请改）。
> **前端待办（选项 C，本次未做）**：`social.jsx`/前端 `api.ts`/`mockApi.ts` 仍指向 `/community/*`，对真后端会 404；待 Phase 2 (b) Figma 落地时改为养宠好文阅读/点赞/收藏页。
